...
def effect_size(measurement1, measurement2):
    # Calculate the means of the two measurements
    mean1 = sum(measurement1) / len(measurement1)
    mean2 = sum(measurement2) / len(measurement2)
    
    # Calculate the standard deviation of the two measurements
    stdev1 = math.sqrt(sum([(x - mean1) ** 2 for x in measurement1]) / len(measurement1))
    stdev2 = math.sqrt(sum([(x - mean2) ** 2 for x in measurement2]) / len(measurement2))
    
    # Calculate Cohen's d
    cohen_d = (mean1 - mean2) / (math.sqrt((stdev1 ** 2 + stdev2 ** 2) / 2))
    return cohen_d

last_uncorrected_p_values = []

# @functools.wraps
def get_params(func):
    """small hack to get access from uncorrected p-values of permutation function"""
    def wrapped(T, H0, tail):
        print('putting uncorrected p values in variable `last_uncorrected_p_values`')
        last_uncorrected_p_values.append(T)
        return func(T, H0, tail)
    return wrapped
...
#     # ugly hack that allows us to access the last p values uncorrected
#     return pval

mne.stats.cluster_level._pval_from_histogram = get_params(_pval_from_histogram)


memory = Memory(config.cache_dir)
# mne.read_epochs = memory.cache(mne.read_epochs)
np.random.seed(0)
...
p = stats.ttest_rel(erps_values[conditions[0]], erps_values[conditions[1]])
d = effect_size(erps_values[conditions[0]], erps_values[conditions[1]])
...
d = effect_size(power_values[conditions[0]], power_values[conditions[1]])
...
for i, (cluster, p) in enumerate(zip(clusters, cluster_pv)):
    if p>0.05: continue
    evoked_diff.plot_image(mask=np.pad(cluster.T, [1,0]), show_names='all')


# calculate statistics for excel report
to_clipboard = ''
for i, (cluster, p) in enumerate(zip(clusters, cluster_pv)):
    p_uncorrected = '' # we cannot report this
    d = X_diff[:, cluster].mean()/X_diff[:, cluster].std()
    time_window_idx = np.where(cluster.max(1))[0][[0, -1]]
    time_window = '-'.join((1000*evoked_diff.times[time_window_idx]).astype(int).astype(str)) + ' ms'
    channels = ', '.join(np.array(ch_names)[cluster.max(0)])
    freqs = ''
    significant = 'not significant' if p>=0.05 else 'significant'
    msg = f'{time_window}\t{channels}\t{freqs}\t{d}\t{p_uncorrected}\t{p}\t{significant}\n'
    to_clipboard += msg
...
to_clipboard = ''
for i, (cluster, p) in enumerate(zip(clusters, cluster_pv)):
    p_uncorrected = t.sf(t_obs[cluster].sum(), 1)
    d = X_diff[:, cluster].mean()/X_diff[:, cluster].mean(1).std()
    time_window_idx = np.where(cluster.max(1).max(0))[0][[0, -1]]
    time_window = '-'.join((1000*avg_trf_diff.times[time_window_idx]).astype(int).astype(str)) + ' ms'
    channels = ', '.join(np.array(ch_names)[cluster.max(0).max(0)])
    freqs_idx = np.where([cluster.max(0).max(1)])[1][[0, -1]]
    freqs = '-'.join(avg_trf_diff.freqs[freqs_idx].astype(str)) + ' Hz'
    print(np.where(avg_trf_diff.freqs[cluster.max(0).max(1)])[0])
    significant = 'not significant' if p>=0.05 else 'significant'
    msg = f'{time_window}\t{channels}\t{freqs}\t{d}\t{p_uncorrected}\t{p}\t{significant}\n'
    to_clipboard += msg
clipboard.copy(to_clipboard)
