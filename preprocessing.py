...
#%% 1.2 Filtering
    ...
    raw.filter(0.1, 100, method='fir', picks=picks_eeg, n_jobs=-1)

    #%% 1.2.2 downsample to 250 Hz, all relevant ERP frequencies should be below that
    if resample:
        print('## resampling')
        raw.resample(250, n_jobs=-1)
...
#%% 1.3.1 ICA: Eyeblinks and horizontal eye movements
    ...
    # else compute it
        ...
        raw_ica = raw.copy().filter(l_freq=1, h_freq=40, n_jobs=-1)
...
#%% 1.2.3 for ERP analysis, apply LP with 35 Hz and Notch with 50 Hz
    print(f'## Epoching')
    raw_erp = raw.copy()
    raw_erp.notch_filter(50, n_jobs=-1)
    ...
    epochs.filter(0, 35, method='fir', n_jobs=-1)
...
for do_resample in [False, True]:
    reports = Parallel(n_jobs=1)(delayed(preprocess)(subj, resample=do_resample) 
